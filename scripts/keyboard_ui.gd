...
func _ready():
	...
	virtual_mouse_pos = get_viewport().size / 2
...
func _unhandled_input(event):
	...
	if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED and event is InputEventMouseMotion:
		...
		virtual_mouse_pos.x = clamp(virtual_mouse_pos.x, 0, viewport.size.x)
		virtual_mouse_pos.y = clamp(virtual_mouse_pos.y, 0, viewport.size.y)

	var is_mouse_event = event is InputEventMouseMotion or event is InputEventMouseButton
